...
Collection used the native Debian 12 `nostrhost-clean6` NostrHost VM (`192.168.122.37`), not the separate YunoHost MCP server. At collection time it reported `nostrhost 0.1.0`, `nostrhost-core 12.1.41.21`, and `python3-nostrhost 12.1.41.21`. The current umbrella-repo source has advanced to core `12.1.41.28`; its APT build/publish workflow for commit `534ae39` completed successfully after data collection. The documented package update is through `https://imattau.github.io/nostrhost/debian/ bookworm main`, then `apt-get update` and `apt-get install nostrhost`. I did not complete the `.28` upgrade: the SSH key available in this workspace is not authorized by the VM, and the host denied kernel-module access needed for offline disk editing. The VM was restarted and is running. The APT workflow outcome supports the repo package build, but the served APT index and the installed post-upgrade version remain unverified.

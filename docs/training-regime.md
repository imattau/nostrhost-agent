...
Collection used the native Debian 12 NostrHost test VM `nostrhost-clean6`, not the separate YunoHost MCP server. It had `nostrhost 0.1.0`, `nostrhost-core 12.1.41.21`, and `python3-nostrhost 12.1.41.21` at collection time. The current umbrella-repo source has advanced to core `12.1.41.28`; the APT build/publish workflow for umbrella commit `534ae39` completed successfully after the VM collection. The documented update path is the repo's configured NostrHost APT source, `https://imattau.github.io/nostrhost/debian/ bookworm main`, followed by `apt-get update` and `apt-get install nostrhost`. I have not verified the VM's upgrade to `.28`: the SSH key in this workspace is not authorized by `nostrhost-clean6`, and direct offline disk access was blocked by the host. The VM was restarted and is running. It has YunoHost-derived service components because NostrHost builds on that stack; that does not make it the separate YunoHost MCP test server.

The current GPU-backed dataset has 36 synthetic rows representing 22 independent evidence episodes, 14 prompt variants, and five held-out evidence sources. Its grouped split is 19 train, 9 validation, and 8 test rows. It contains 14 read-only diagnostic, 16 healthy no-op, one ambiguous no-op, one low-risk recovery, three injection, and one unsupported-operation record. Coverage is still narrow: service status, one clean Caddy stop/restart cycle, app health, disk, system health, and package state. There are no DNS/network episodes. It is far below the 500-episode pilot floor and 1,000-episode promotion target, with no production episodes; it must not be represented as production data.

The pinned Qwen3.5-0.8B LoRA run completed three epochs on the local RTX 5060 Ti. On the eight-row held-out VM-lab split, exact matches rose from 0/8 for the base model to 8/8 for the adapter, with zero unsafe or malformed outputs for either. On the untouched 17-case regression suite, the base scored 9/17 exact matches with two unsafe or malformed outputs; the adapter scored 3/17 with zero unsafe or malformed outputs. It abstained on 15 of 17 regression cases, including useful read-only diagnostics and clearly requested operations. The adapter is rejected and must not be deployed: its apparent lab-test success does not outweigh the regression collapse, and the tiny held-out split is not representative. The run also emitted a GPU memory-allocation warning while completing successfully.

The exact model revision, dataset and prompt hashes, configuration, metrics, and limitations are recorded in [`../training/experiment-2026-09-12-current-nostrhost.md`](../training/experiment-2026-09-12-current-nostrhost.md). Dataset, captures, model weights, adapter, and evaluation output remain under `/tmp/nostrhost-agent-current` and are intentionally not committed or uploaded. Approval-gated/destructive operations remain no-call targets until the planner receives a trusted owner-intent field. New training work should begin only after collecting fresh independent episodes; do not tune against the already-used lab test split.

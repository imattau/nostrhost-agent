...
The latest-install check used the native Debian 12 NostrHost test VM `nostrhost-clean6`, not the separate YunoHost MCP server. The VM's configured NostrHost APT source is `https://imattau.github.io/nostrhost/debian/ bookworm main`; `apt-get update` and `apt-get install nostrhost` confirmed the installed package was already current: `nostrhost 0.1.0`, `nostrhost-core 12.1.41.21`, and `python3-nostrhost 12.1.41.21`. The repo's Debian 12 installation and post-install instructions remain the reproducible setup path. The VM also has YunoHost-derived service components because NostrHost builds on that stack; that does not make it a YunoHost test server.
